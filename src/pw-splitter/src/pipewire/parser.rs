...
#[cfg(test)]
mod unused_but_potentially_useful {
    use super::*;

    /// Get FL/FR output ports for a node
    #[allow(dead_code)]
    pub fn get_stereo_output_ports(
        node_id: u32,
        ports: &[AudioPort],
    ) -> (Option<AudioPort>, Option<AudioPort>) {
        let node_ports: Vec<_> = ports
            .iter()
        ...
        let fl = node_ports
            .iter()
            .find(|p| p.channel == "FL" || p.port_name.contains("FL"))
            .cloned()
            .cloned();
        let fr = node_ports
            .iter()
            .find(|p| p.channel == "FR" || p.port_name.contains("FR"))
            .cloned()
            .cloned();
    ...
    /// Get FL/FR input ports for a node
    #[allow(dead_code)]
    pub fn get_stereo_input_ports(
        node_id: u32,
        ports: &[AudioPort],
    ) -> (Option<AudioPort>, Option<AudioPort>) {
        let node_ports: Vec<_> = ports
